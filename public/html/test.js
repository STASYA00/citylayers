...
let width = 400;
let height = 400;
let radius = 0.9 * Math.min(width, height) / 2;
...
let vis = d3.select("#section-project").append("svg:svg")
  .attr("width", width)
  .attr("height", height)
  .append("svg:g")
  .attr("id", "container")
...
function createVisualization(json) {
  ...
  let path = vis.data([json]).selectAll("path");
    ...
    .attr("d", arc)
    .attr("id", function(d,i) { return "piearc_" + i; })
    ...
    .style("fill", function(d) {
        if (d.depth==2){
          ...
          return colors[d.parent.parent.name];
        }
        else {
  ...
  vis.data([json]).selectAll("path");
    ...
    .enter().append("text")
        ...
        .attr("font-size", "12px")
        .append("textPath")
        .attr("xlink:href",function(d,i){return "#piearc_"+i;})
