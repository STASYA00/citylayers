...
<div class="main-wrapper" id="main-container" x-data>
    ...
    <template x-if="$store.data.step.current == 0">
        <section>
            ...
            <p>
                Begin by adding a photo of the place you want to share. Then, follow a few steps to 
            share your thoughts about this place with others. Once completed, your contribution will be 
            displayed on the map for everyone to see.
            </p>
            ...
            <div>
                ...
                <span class="skip-span" 
                        ...
                        x-show="!$store.data.place_data['img']">
                    or skip
                </span>
            </div>
        </section>
    ...
    <section>
        ...
        <template x-if="$store.data.step.current == 1">
                <h1></h1>
                <div><p>Σε αυτό το σημείο αισθάνομαι</p> <input name="feeling" type="text" /></div>
        </template>
        <template x-if="$store.data.step.current == 0">
            <div>
                <label for="img-uploader">
    ...
    <template x-cloak x-if="$store.data.step.current > 0 && $store.data.step.current < $store.data.step.length">
        <section>
            <div class="question-container">
                ...
                <div x-html="$store.data.copy_data.filter(c=>c.id==$store.data.step.current)[0].description"></div>
...
<script>
    ...
    function submitData(place_data) {
        ...
        if (place_data["feeling"]!=undefined && place_data["feeling"]!=null && place_data["feeling"]!=""){
            d.set("feeling", place_data["feeling"].toString());
        }
